...
public class JlrBlockLightEngine {
	...
	protected IAlphaProvider getFastestPreviousAlphaProvider(BlockState[] oldbss, long[] targets, int size, TaskCache taskCache) {
		...
		return (xyz, quadr, hol) -> getAlphas(xyz, bsprov, quadr, hol, taskCache);
	}
	 ...
	 */
	public static AlphaHolder getAlphas(Vector3i xyz, IBlockStateProvider bsprov, Quadrant quadr, AlphaHolder hol, TaskCache taskCache) {
		BlockState state = bsprov.get(xyz);
		hol.f1 = hol.f2 = hol.f3 = hol.f4 = hol.f5 = hol.f6 = hol.block = getAlpha(state, taskCache, xyz.x, xyz.y, xyz.z);
		if (hol.block != 0 && isntEmptyShape(state)) {
			...
			// previous
			hol.f1 = getFaceAlpha(state, bsprov, d1, xyz.x - quadr.axis1.x, xyz.y, xyz.z, taskCache, xyz) * hol.block;
			hol.f2 = getFaceAlpha(state, bsprov, d2, xyz.x, xyz.y - quadr.axis2.y, xyz.z, taskCache, xyz) * hol.block;
			hol.f3 = getFaceAlpha(state, bsprov, d3, xyz.x, xyz.y, xyz.z - quadr.axis3.z, taskCache, xyz) * hol.block;

			// next
			hol.f4 = getFaceAlpha(state, bsprov, d4, xyz.x + quadr.axis1.x, xyz.y, xyz.z, taskCache, xyz) * hol.block;
			hol.f5 = getFaceAlpha(state, bsprov, d5, xyz.x, xyz.y + quadr.axis2.y, xyz.z, taskCache, xyz) * hol.block;
			hol.f6 = getFaceAlpha(state, bsprov, d6, xyz.x, xyz.y, xyz.z + quadr.axis3.z, taskCache, xyz) * hol.block;
		}
		return hol;
	}
	 ...
	 */
	protected static float getFaceAlpha(BlockState curstate, IBlockStateProvider bsprov, Direction dir, int ox, int oy, int oz, TaskCache taskCache, Vector3i xyz) {
