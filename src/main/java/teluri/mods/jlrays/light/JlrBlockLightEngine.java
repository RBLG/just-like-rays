...
public class JlrBlockLightEngine {
	...
	public void checkBlock(BlockPos pos) {
		if (!(pos instanceof ShinyBlockPos)) {
			if (!(pos instanceof DullBlockPos)) { // skip warn if already caught
				DullBlockPos.warn();
			}
			return;
		}
		...
		TaskCache tc = this.taskCacheFactory.createWithRange(pos, 0);
		int x = pos.getX(), y = pos.getY(), z = pos.getZ();
		if (getAlpha(prev, tc, x, y, z) != getAlpha(curr, tc, x, y, z) || prev.useShapeForLightOcclusion() || curr.useShapeForLightOcclusion()) {
	...
	public void runLightUpdates() {
		...
		changeMap.forEach((longpos, prev) -> {
			BlockState curr = this.blockStateProvider.get(longpos);
			if (getAlpha(curr, tc, BlockPos.getX(longpos), BlockPos.getY(longpos), BlockPos.getZ(longpos)) == 0 && curr.getLightEmission() == 0) {
	...
	protected IAlphaChangeProvider getFastestPreviousAlphaProvider(BlockState[] oldbss, long[] targets, int size, TaskCache taskCache) {
		...
		return (xyz, source, quadr, ohol, nhol) -> getAlphasChange(xyz, source, bsprov, taskCache, quadr, ohol, nhol, taskCache);
	}
	 ...
	 */
	public static int getAlpha(BlockState state, TaskCache taskCache, int x, int y, int z) {
	...
	public static AlphaHolder getAlphas(Vector3i xyz, Vector3i source, IBlockStateProvider bsprov, Quadrant quadr, AlphaHolder hol, TaskCache taskCache) {
		...
		hol.setAll(getAlpha(state, taskCache, xyz.x, xyz.y, xyz.z));
		if (hol.block != 0 && isntEmptyShape(state)) {
			...
			hol.f1 *= getFaceAlpha(source, state, bsprov, d1, xyz.x - quadr.axis1.x, xyz.y, xyz.z, taskCache, xyz);
			hol.f2 *= getFaceAlpha(source, state, bsprov, d2, xyz.x, xyz.y - quadr.axis2.y, xyz.z, taskCache, xyz);
			hol.f3 *= getFaceAlpha(source, state, bsprov, d3, xyz.x, xyz.y, xyz.z - quadr.axis3.z, taskCache, xyz);

			// next
			hol.f4 *= getFaceAlpha(source, state, bsprov, d4, xyz.x + quadr.axis1.x, xyz.y, xyz.z, taskCache, xyz);
			hol.f5 *= getFaceAlpha(source, state, bsprov, d5, xyz.x, xyz.y + quadr.axis2.y, xyz.z, taskCache, xyz);
			hol.f6 *= getFaceAlpha(source, state, bsprov, d6, xyz.x, xyz.y, xyz.z + quadr.axis3.z, taskCache, xyz);
		}
		return hol;
	}
	...
	public static void getAlphasChange(Vector3i xyz, Vector3i source, IBlockStateProvider oprov, IBlockStateProvider nprov, Quadrant quadr, AlphaHolder ohol,
			AlphaHolder nhol, TaskCache taskCache) {
		BlockState nstate = nprov.get(xyz);
		BlockState ostate = oprov.get(xyz);
		int nalpha = getAlpha(nstate, taskCache, xyz.x, xyz.y, xyz.z);
		nhol.setAll(nalpha);
		...
		if (same) {
			...
			ohol.setAll(getAlpha(ostate, taskCache, xyz.x, xyz.y, xyz.z));
		...
		if (ncomplex) {
			// previous
			nhol.f1 *= getFaceAlpha(source, nstate, nprov, d1, x1___, xyz.y, xyz.z, taskCache, xyz);
			nhol.f2 *= getFaceAlpha(source, nstate, nprov, d2, xyz.x, y2___, xyz.z, taskCache, xyz);
			nhol.f3 *= getFaceAlpha(source, nstate, nprov, d3, xyz.x, xyz.y, z3___, taskCache, xyz);

			// next
			nhol.f4 *= getFaceAlpha(source, nstate, nprov, d4, x4___, xyz.y, xyz.z, taskCache, xyz);
			nhol.f5 *= getFaceAlpha(source, nstate, nprov, d5, xyz.x, y5___, xyz.z, taskCache, xyz);
			nhol.f6 *= getFaceAlpha(source, nstate, nprov, d6, xyz.x, xyz.y, z6___, taskCache, xyz);
		}
		if (same) {
			ohol.setAll(nhol);
		} else if (ocomplex) {
			// previous
			ohol.f1 *= getFaceAlpha(source, ostate, oprov, d1, x1___, xyz.y, xyz.z, taskCache, xyz);
			ohol.f2 *= getFaceAlpha(source, ostate, oprov, d2, xyz.x, y2___, xyz.z, taskCache, xyz);
			ohol.f3 *= getFaceAlpha(source, ostate, oprov, d3, xyz.x, xyz.y, z3___, taskCache, xyz);

			// next
			ohol.f4 *= getFaceAlpha(source, ostate, oprov, d4, x4___, xyz.y, xyz.z, taskCache, xyz);
			ohol.f5 *= getFaceAlpha(source, ostate, oprov, d5, xyz.x, y5___, xyz.z, taskCache, xyz);
			ohol.f6 *= getFaceAlpha(source, ostate, oprov, d6, xyz.x, xyz.y, z6___, taskCache, xyz);
		}
	}
	...
	protected static float getFaceAlpha(Vector3i source, BlockState curstate, IBlockStateProvider bsprov, Direction dir, int ox, int oy, int oz,
			TaskCache taskCache, Vector3i xyz) {
		BlockState otherstate = source.equals(ox, oy, oz) ? Blocks.AIR.defaultBlockState() : bsprov.getState(ox, oy, oz);
		return shapeOccludes(curstate, otherstate, dir, taskCache, ox, oy, oz, xyz) ? 0 : 1;
	}
	...
	public static boolean shapeOccludes(BlockState state1, BlockState state2, Direction dir, TaskCache taskCache, int ox, int oy, int oz, Vector3i xyz) {
		VoxelShape voxelShape = taskCache.getOcclusionShape(state1, dir, xyz.x, xyz.y, xyz.z);
		VoxelShape voxelShape2 = taskCache.getOcclusionShape(state2, dir.getOpposite(), ox, oy, oz);
	...
	public int calculateLightLevel(float visi, float distinv, int emit) {
		...
		return emit == 0 || visi == 0 ? 0 : Mth.clamp((int) ((visi * distinv * emit - correctedMinimumValue) * minValCorrector2), 0, emit);
	}
